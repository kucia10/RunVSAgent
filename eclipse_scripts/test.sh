...
# Test script for RunVSAgent Eclipse plugin project
...
show_help() {
    cat << EOF
$SCRIPT_NAME - Run tests for RunVSAgent Eclipse plugin project
...
run_env_tests() {
    ...
    local required_dirs=("$EXTENSION_HOST_DIR" "$ECLIPSE_DIR" "eclipse_scripts")
    ...
    # Test 4: Check Maven availability
    log_info "Testing Maven availability..."
    if command_exists "mvn"; then
        log_success "✓ Maven availability test passed"
    else
...
run_lint_tests() {
    ...
    if command_exists "shellcheck"; then
        ...
        local shell_files
        shell_files=$(find "$PROJECT_ROOT/eclipse_scripts" -name "*.sh" -type f)
...
run_build_tests() {
    ...
    local required_files=(
        ...
        "$PROJECT_ROOT/$ECLIPSE_DIR/pom.xml"
    )
...
generate_test_report() {
    ...
    cat > "$report_file" << EOF
RunVSAgent Eclipse Plugin Test Report
======================================
...
- Maven Version: $(mvn --version 2>/dev/null | head -n 1 || echo "Not found")
...
# Main test function
main() {
    log_info "Starting RunVSAgent Eclipse plugin test process..."
