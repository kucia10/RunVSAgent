...
# Setup script for RunVSAgent Eclipse plugin project
...
show_help() {
    cat << EOF
$SCRIPT_NAME - Setup development environment for RunVSAgent Eclipse plugin
...
DESCRIPTION:
    ...
    - Validating system requirements (including Git LFS and Maven)
...
REQUIREMENTS:
    ...
    - Maven must be installed for Eclipse plugin build
...
validate_system_requirements() {
    ...
    local dev_tools=("git" "unzip" "curl" "mvn")
    ...
    if command_exists "mvn"; then
        ...
        log_debug "Found Maven: $maven_version"
    else
        log_warn "Maven not found. Eclipse plugin build will fail."
    fi
...
verify_setup() {
    ...
    if [[ -d "$PROJECT_ROOT/$ECLIPSE_DIR" ]]; then
        if [[ ! -f "$PROJECT_ROOT/$ECLIPSE_DIR/pom.xml" ]]; then
            log_warn "No pom.xml file found in Eclipse directory"
        fi
    fi
...
# Main setup function
main() {
    log_info "Starting RunVSAgent Eclipse plugin development environment setup..."
    ...
    log_info ""
    log_info "Next steps:"
    log_info "  1. Run './eclipse_scripts/build.sh' to build the project"
    log_info "  2. Run './eclipse_scripts/build.sh --help' for build options"
    log_info "  3. Check the generated files in the eclipse_plugin/ directory"
    log_info ""
