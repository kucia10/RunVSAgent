...
# Main entry point script for RunVSAgent Eclipse plugin project
...
show_main_help() {
    cat << EOF
$SCRIPT_NAME - RunVSAgent Eclipse Plugin Project Management Tool

USAGE:
    $SCRIPT_NAME <command> [options]

DESCRIPTION:
    This is the main entry point for RunVSAgent Eclipse plugin project operations.
...
PROJECT STRUCTURE:
    extension_host/     Base extension runtime
    eclipse_plugin/     Eclipse plugin source
    deps/               Dependencies and submodules
    eclipse_scripts/    Build and maintenance scripts
...
RunVSAgent Eclipse Plugin Project Management Tool
...
main() {
    ...
    if [[ $# -eq 0 ]]; then
        log_info "RunVSAgent Eclipse Plugin Project Management Tool v$SCRIPT_VERSION"
