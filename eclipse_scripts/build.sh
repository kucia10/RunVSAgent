...
# Build script for RunVSAgent Eclipse plugin project
# This script builds VSCode extension and Eclipse plugin
...
readonly TARGET_ECLIPSE="eclipse"
...
show_help() {
    cat << EOF
$SCRIPT_NAME - Build RunVSAgent Eclipse plugin project components
...
OPTIONS:
    ...
    --skip-eclipse        Skip Eclipse plugin build
...
ENVIRONMENT:
    ...
    SKIP_ECLIPSE_BUILD  Skip Eclipse build if set to 'true'
...
parse_build_args() {
    ...
    case "$BUILD_TARGET" in
        "$TARGET_ALL"|"$TARGET_VSCODE"|"$TARGET_BASE"|"$TARGET_ECLIPSE")
            ;;
        *)
            log_error "Invalid build target: $BUILD_TARGET"
            log_info "Valid targets: $TARGET_ALL, $TARGET_VSCODE, $TARGET_BASE, $TARGET_ECLIPSE"
            exit 3
            ;;
    esac
    ...
    [[ "${SKIP_ECLIPSE_BUILD:-false}" == "true" ]] && SKIP_ECLIPSE_BUILD=true
...
validate_build_environment() {
    ...
    if [[ ! -d "$vscode_dir" ]] || [[ ! "$(ls -A "$vscode_dir" 2>/dev/null)" ]]; then
        log_error "VSCode submodule not initialized. Run './eclipse_scripts/setup.sh' first."
        exit 4
    fi
    ...
    if [[ "$SKIP_BASE_BUILD" != "true" ]]; then
        if [[ ! -f "$PROJECT_ROOT/$EXTENSION_HOST_DIR/package.json" ]]; then
            log_error "Base package.json not found. Run './eclipse_scripts/setup.sh' first."
            exit 4
        fi
    fi
    
    if [[ "$SKIP_ECLIPSE_BUILD" != "true" ]]; then
        if [[ ! -f "$PROJECT_ROOT/$ECLIPSE_DIR/pom.xml" ]]; then
            log_error "Eclipse pom.xml not found."
            exit 4
        fi
    fi
...
build_eclipse_component() {
    ...
    log_success "Eclipse plugin built"
}
...
copy_build_artifacts() {
    ...
    # Copy Eclipse plugin
    if [[ -n "${ECLIPSE_PLUGIN_FILE:-}" && -f "$ECLIPSE_PLUGIN_FILE" ]]; then
        copy_files "$ECLIPSE_PLUGIN_FILE" "$OUTPUT_DIR/" "Eclipse plugin"
    fi
...
show_build_summary() {
    ...
    # Show Eclipse plugin
    if [[ -n "${ECLIPSE_PLUGIN_FILE:-}" && -f "$ECLIPSE_PLUGIN_FILE" ]]; then
        log_info "  Eclipse Plugin: $ECLIPSE_PLUGIN_FILE"
    fi
    ...
    echo ""
    log_info "Next steps:"
    if [[ "$BUILD_TARGET" == "$TARGET_ALL" || "$BUILD_TARGET" == "$TARGET_ECLIPSE" ]]; then
        log_info "  1. Install Eclipse plugin from: ${ECLIPSE_PLUGIN_FILE:-$ECLIPSE_BUILD_DIR/target/}"
        log_info "  2. Configure plugin settings in Eclipse"
    fi
...
# Main build function
main() {
    log_info "Starting RunVSAgent Eclipse build process..."
