...
# Build-specific utility functions for Eclipse plugin
# This file provides functions for building VSCode extensions and Eclipse plugins
...
readonly ECLIPSE_DIR="eclipse_plugin"
...
init_build_env() {
    ...
    export ECLIPSE_BUILD_DIR="$PROJECT_ROOT/$ECLIPSE_DIR"
    export VSCODE_PLUGIN_NAME="${VSCODE_PLUGIN_NAME:-roo-code}"
    export VSCODE_PLUGIN_TARGET_DIR="$ECLIPSE_BUILD_DIR/plugins/${VSCODE_PLUGIN_NAME}"
    ...
    log_debug "Eclipse build directory: $ECLIPSE_BUILD_DIR"
...
validate_build_tools() {
    ...
    local required_tools=("git" "node" "npm" "unzip" "mvn")
    ...
    # Check for Maven (for Eclipse plugin)
    if command_exists "mvn"; then
        local mvn_version
        mvn_version=$(mvn -v 2>/dev/null | head -n 1)
        log_debug "Found Maven: $mvn_version"
    else
        log_warn "Maven not found, Eclipse plugin build may fail"
    fi
...
copy_debug_resources() {
    ...
    # Copy Eclipse themes if they exist
    local eclipse_themes_dir="$ECLIPSE_BUILD_DIR/src/main/resources/themes"
    if [[ -d "$eclipse_themes_dir" ]]; then
        copy_files "$eclipse_themes_dir/*" "$vscode_plugin_debug_dir/src/integrations/theme/default-themes/" "Eclipse themes"
    fi
...
build_extension_host() {
    ...
    execute_cmd "npm ls --prod --depth=10 --parseable > '$ECLIPSE_BUILD_DIR/prodDep.txt'" "production dependencies list"
...
build_eclipse_plugin() {
    if [[ "$SKIP_ECLIPSE_BUILD" == "true" ]]; then
        log_info "Skipping Eclipse plugin build"
        return 0
    fi
    
    log_step "Building Eclipse plugin..."
    
    cd "$ECLIPSE_BUILD_DIR"
    
    # Check for Maven POM file
    if [[ ! -f "pom.xml" ]]; then
        die "No pom.xml file found in Eclipse directory"
    fi
    
    # Set build properties based on BUILD_MODE
    local maven_opts=""
    if [[ "$BUILD_MODE" == "$BUILD_MODE_RELEASE" ]]; then
        maven_opts="-DskipTests"
        log_info "Building Eclipse plugin in release mode"
    elif [[ "$BUILD_MODE" == "$BUILD_MODE_DEBUG" ]]; then
        maven_opts="-Ddebug=true"
        log_info "Building Eclipse plugin in debug mode"
    fi
    
    # Build plugin with Maven/Tycho
    execute_cmd "mvn clean verify $maven_opts" "Eclipse plugin build"
    
    # Find generated plugin
    local plugin_file
    plugin_file=$(find "$ECLIPSE_BUILD_DIR" -path "*/target/*.jar" -type f | grep -v "sources.jar" | sort -r | head -n 1)
    
    if [[ -n "$plugin_file" ]]; then
        log_success "Eclipse plugin built: $plugin_file"
        export ECLIPSE_PLUGIN_FILE="$plugin_file"
    else
        log_warn "Eclipse plugin file not found in target directories"
    fi
}
