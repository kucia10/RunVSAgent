...
# Clean script for RunVSAgent Eclipse plugin project
...
show_help() {
    cat << EOF
$SCRIPT_NAME - Clean RunVSAgent Eclipse plugin project artifacts
...
DESCRIPTION:
    ...
    - Cache files (npm, maven, temp files)
    - Log files
    - Temporary files
...
clean_build_artifacts() {
    ...
    # Clean Eclipse plugin build
    local eclipse_dir="$PROJECT_ROOT/$ECLIPSE_DIR"
    if [[ -d "$eclipse_dir" ]]; then
        log_info "Cleaning Eclipse plugin build..."
        # Clean all target directories in Eclipse plugin modules
        find "$eclipse_dir" -name "target" -type d -exec rm -rf {} + 2>/dev/null || true
        remove_dir "$eclipse_dir/plugins/roo-code"
        [[ -f "$eclipse_dir/prodDep.txt" ]] && rm -f "$eclipse_dir/prodDep.txt"
    fi
...
clean_dependencies() {
    ...
    # Clean Maven local repository (Eclipse plugin dependencies)
    if [[ -d "$HOME/.m2/repository" ]]; then
        log_info "Cleaning Maven local repository cache..."
        if [[ "$DRY_RUN" == "true" ]]; then
            log_debug "Would clean: $HOME/.m2/repository"
        else
            # Clean only temp files and lock files, not the entire repository
            find "$HOME/.m2/repository" -type f -name "*.lock" -delete 2>/dev/null || true
            find "$HOME/.m2/repository" -type d -name "_remote.repositories" -exec rm -rf {} + 2>/dev/null || true
        fi
    fi
...
clean_logs() {
    ...
    local eclipse_dir="$PROJECT_ROOT/$ECLIPSE_DIR"
    if [[ -d "$eclipse_dir" ]]; then
        find "$eclipse_dir" -name "*.log" -type f -exec rm -f {} \; 2>/dev/null || true
    fi
...
show_clean_summary() {
    ...
    if [[ "$CLEAN_TARGET" == "$TARGET_ALL" || "$CLEAN_TARGET" == "$TARGET_DEPS" ]]; then
        log_info "  1. Run './eclipse_scripts/setup.sh' to reinstall dependencies"
        log_info "  2. Run './eclipse_scripts/build.sh' to rebuild the project"
    elif [[ "$CLEAN_TARGET" == "$TARGET_BUILD" ]]; then
        log_info "  1. Run './eclipse_scripts/build.sh' to rebuild the project"
    fi
...
# Main clean function
main() {
    log_info "Starting RunVSAgent Eclipse clean process..."
