...
// import com.sina.weibo.agent.eclipse.neo.plugin.DEBUG_MODE
// import com.sina.weibo.agent.eclipse.neo.plugin.WecoderPluginService
import com.sina.weibo.agent.eclipse.neo.util.PluginResourceUtil
...
class ExtensionProcessManager {
    companion object {
        // Node modules path
        private const val NODE_MODULES_PATH = "resources/node_modules"

        // Extension process entry file
        private const val EXTENSION_ENTRY_FILE = "extension.js"

        // Runtime directory
        private const val RUNTIME_DIR = "resources/runtime"

        // Plugin ID
        private const val PLUGIN_ID = "com.sina.weibo.agent.eclipse.neo"
    ...
    private fun LOG(level: String, message: String, e: Exception? = null) {
        println("[$level] ExtensionProcessManager: $message")
        e?.printStackTrace()
    }
...
//                "--trace-uncaught",
    ...
    private fun findExecutableInPath(name: String): String? {
        ...
        val paths = systemPath.split(separator)
        for (path in paths) {
            val executable = File(path, name)
            if (executable.exists() && executable.canExecute()) {
                LOG("INFO", "Found executable in PATH: ${executable.absolutePath}")
                return executable.absolutePath
            }
        }
        LOG("WARN", "$name not found in system PATH")
        return null
    }
