...
class ExtensionManager {
    private val bundle = FrameworkUtil.getBundle(ExtensionManager::class.java)
    private val LOG = Platform.getLog(bundle)
    ...
    fun initialize(configuredExtensionId: String? = null) {
        ...
        if (configuredExtensionId != null) {
            ...
            if (provider != null && provider.isAvailable(null)) {
    ...
    fun isConfigurationValid(): Boolean {
        return currentProvider != null && currentProvider!!.isAvailable(null)
    }
    ...
    fun getConfigurationError(): String? {
        ...
        } else if (!currentProvider!!.isAvailable(null)) {
    ...
    private fun setDefaultExtensionProvider() {
        ...
        val availableProviders = extensionProviders.values.filter { it.isAvailable(null) }
...
//             val rooProvider = availableProviders.find { it.getExtensionId() == "roo-code" }
    ...
    fun setCurrentProvider(extensionId: String, forceRestart: Boolean? = false): Boolean {
        ...
        if (provider != null && provider.isAvailable(null)) {
            ...
            try {
                val configManager = ExtensionConfigurationManager.getInstance()
            ...
            try {
                if (forceRestart == false) {
                    val buttonManager = DynamicButtonManager.getInstance()
            ...
            try {
                if (forceRestart == false) {
                    val contextMenuManager = com.sina.weibo.agent.extensions.ui.contextmenu.DynamicContextMenuManager.getInstance()
            ...
            try {
                // TODO: Implement event bus in Eclipse.
    ...
    fun switchExtensionProvider(extensionId: String, forceRestart: Boolean = false): CompletableFuture<Boolean> {
        val extensionSwitcher = ExtensionSwitcher.getInstance()
    ...
    fun getAvailableProviders(): List<ExtensionProvider> {
        return extensionProviders.values.filter { it.isAvailable(null) }
    }
    ...
    fun initializeCurrentProvider() {
        currentProvider?.initialize(null)
    }
